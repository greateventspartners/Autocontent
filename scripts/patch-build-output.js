  ...
  if (code.includes('.body("Internal Server Error").send()')) {
    ...
    for (const m of fullMatches) {
      ...
      if (logErrMatch) {
        ...
        const replacement = `${resp}.statusCode=500,${resp}.setHeader("x-error-msg",String(${errVar}?.message||${errVar})),${resp}.setHeader("x-error-stack",String(${errVar}?.stack||"no-stack").substring(0,500)),${resp}.body("Internal Server Error").send()`;
